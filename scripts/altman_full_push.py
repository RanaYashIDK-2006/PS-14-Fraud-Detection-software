#!/usr/bin/env python3
"""Load ALL 24M Altman rows (all fraud, sample legit) + Optuna push.

Previous runs with 16M rows → 99.48%. Loading all 24M gives ~29K fraud (vs 19K).
"""
...
# ── Load ALL Altman rows ──
print("  Loading ALL Altman rows (24M)...")
t_load = time.time()
...
for chunk in pd.read_csv("data/credit_card_transactions-ibm_v2.csv",
    ...
    chunk["amt_bin"] = pd.cut(chunk["amt_log"].replace([np.inf,-np.inf], np.nan), bins=20, labels=False).astype(float)

    fraud_mask = chunk["label"] == 1
    fraud_df = chunk[fraud_mask]
    # Sample 2% of legit to keep total manageable
    legit_sample = chunk[~fraud_mask].sample(frac=0.02, random_state=rng) if (~fraud_mask).sum() > 0 else chunk[~fraud_mask]
    rows_all.append(pd.concat([fraud_df, legit_sample]))
    ci += 1
    # Process all chunks but limit to 24
    if ci >= 24: break
    if ci % 5 == 0: print(f"    ...chunk {ci} ({time.time()-t_load:.0f}s)")
...
result = dict(dataset="ALTMAN_FULL_PUSH", n_rows=len(y), n_features=len(cols),
