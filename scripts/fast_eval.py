#!/usr/bin/env python3
"""Speed-optimized ML eval: n_jobs=4, incremental saves, subsampled Altman.

Key fixes for Windows 16-core bottleneck:
  - n_jobs=4 (not -1) to avoid thread thrashing
  - 200-tree XGB max (diminishing returns beyond that)
  - Altman limited to --max-rows (default 200K) to avoid 2.2GB load
  - Each dataset result saved immediately (no lost progress on timeout)
"""
...
def load_altman(max_rows=200_000):
    """Load Altman single-pass with subsampling.
    
    Known: 24.4M rows, ~0.12% fraud (29,757 fraud rows).
    Strategy: keep ALL fraud rows, sample ~0.5% of legit.
    """
    print(f"  Loading Altman (max {max_rows:,} rows, single-pass)...")
    t0 = time.time()
    
    # Single pass: keep all fraud, sample 0.5% of legit
    ...
    df["amt_sq"] = df["amt"]**2
    y = df["label"].values
    cols = [c for c in df.columns if c not in ("label","User","Time","Amount","Use Chip","MCC","Errors?","Is Fraud?")]
    return np.nan_to_num(df[cols].values.astype(np.float32)), y, cols
...
def evaluate(name, X, y, xgb_p, lgb_p):
    ...
    for j, (nm, m) in enumerate(models):
        ...
        m2.fit(Xtr[:half], ytr[:half])
        meta_tr[:,j] = m2.predict_proba(Xtr[half:])[:,1]
        meta_te[:,j] = m2.predict_proba(Xte)[:,1]
    ...
    out = R/f"fast_eval_{name.lower()}.json"
...
if __name__ == "__main__":
    ...
    p.add_argument("--max-rows", type=int, default=200_000, help="Max rows for Altman (default 200K)")
    a = p.parse_args()
    t0 = time.time()
    res = {}
