#!/usr/bin/env python3
"""
Full 24M Altman with graph+velocity features.
Optimized: load only needed columns, chunked feature engineering.
Baseline: 15 features → 0.9462 AUC (temporal split).
Target: Add graph+velocity → push above 0.9500 AUC.
"""
...
print("="*80)
print("ALTMAN — Full 24M Graph+Velocity")
print("="*80)
t_total = time.time()

# ═══ Phase 1: Two-pass loading ═══
# Pass 1: Load and get temporal split boundary
print("  Pass 1: Loading to find split boundary...")
t0 = time.time()
n_rows = 0
for chunk in pd.read_csv("data/credit_card_transactions-ibm_v2.csv", usecols=USECOLS, low_memory=False, chunksize=1_000_000):
    n_rows += len(chunk)
...
# Pass 2: Load all data, parse on the fly
print("\n  Pass 2: Loading with feature parsing...")
t0 = time.time()
all_rows = []
nloaded = 0
for chunk in pd.read_csv("data/credit_card_transactions-ibm_v2.csv", usecols=USECOLS, low_memory=False, chunksize=500_000):
    c = pd.DataFrame()
    c["User"] = chunk["User"]
    c["Card"] = chunk["Card"]
    ...
    all_rows.append(c)
    nloaded += len(c)
    if nloaded % 2_000_000 == 0:
        print(f"    {nloaded:,} loaded", end="\r")

df = pd.concat(all_rows, ignore_index=True)
print(f"  Loaded {len(df):,} ({df['label'].sum():,} fraud) ({time.time()-t0:.1f}s)")

# Temporal split
tr = df.iloc[:split_idx].copy(); te = df.iloc[split_idx:].copy()
...
# ═══ Phase 2: Build stats from train ═══
print("\n  Building stats...")
t0 = time.time()
...
utx=us["uc"].to_dict(); ufr=us["ufr"].to_dict(); uavg=us["ua"].to_dict(); usd=us["usd"].fillna(1).to_dict(); umrc=us["um"].to_dict(); ucit=us["uci"].to_dict()
...
def eng(dp):
    ...
    o["zzip"]=dp["zzip"]; o["state"]=(dp["state"]!="").astype(int); o["month"]=dp["month"]

    o["mf"]=dp["merchant"].map(mfrq).fillna(0)
    o["mfr"]=dp["merchant"].map(mm).fillna(gm)
    o["mavg"]=dp["merchant"].map(mavg).fillna(dp["amt"].mean())
    o["mstd_v"]=dp["merchant"].map(mstd).fillna(1)
    o["avm"]=dp["amt"]/o["mavg"].clip(lower=0.01)

    o["cfr"]=dp["city"].map(cm).fillna(gm); o["cfq"]=dp["city"].map(cfq).fillna(0)

    uid=dp["User"]
    o["utc"]=uid.map(utx).fillna(0); o["ufr_val"]=uid.map(ufr).fillna(gm)
    o["uavg"]=uid.map(uavg).fillna(dp["amt"].mean()); o["usd_v"]=uid.map(usd).fillna(1)
    ...
    # Graph
    o["umd"]=uid.map(umdeg).fillna(0); o["mud"]=dp["merchant"].map(mudeg).fillna(0)
    o["mun"]=dp["merchant"].map(munq).fillna(1)
    o["ucd"]=uid.map(ucdeg).fillna(0); o["cun"]=dp["city"].map(cunq).fillna(1)
    mx=max(o["umd"].max(),1); o["ucen"]=o["umd"]/mx
    mx2=max(o["mud"].max(),1); o["mpop"]=o["mud"]/mx2
    o["mfr_x_amt"]=o["mfr"]*dp["amt"]; o["mfr_x_ufr"]=o["mfr"]*o["ufr_val"]
    ...
    o["axur"]=dp["amt"]*o["ufr_val"]; o["axmr"]=dp["amt"]*o["mfr"]
...
# Threshold sweep
print(f"\n  Threshold Sweep:")
print(f"  {'Thr':>6}|{'FPR':>8}|{'Recall':>8}|{'Prec':>8}")
print("  "+("-"*40))
for thr in [0.1, 0.25, 0.5, 0.75, 1.0]:
    yp=(p_ens>=thr).astype(int)
    tp=int(((yp==1)&(yte==1)).sum()); fp=int(((yp==1)&(yte==0)).sum())
    fn=int(((yp==0)&(yte==1)).sum())
    rec=tp/max(nft,1); fpr=fp/max(nnt,1); pr=tp/max(tp+fp,1)
    print(f"  {thr:>6.2f}|{fpr:>7.4f}|{rec:>7.4f}|{pr:>7.4f}")
...
out={"dataset":"ibm_altman_full_graph_velocity","n_total":len(df),"n_train":len(tr),"n_test":len(te),
...
with open("reports/altman_full_graph_velocity.json","w") as f:
