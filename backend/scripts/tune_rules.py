...
def modify_rules(base_path: Path, modifications: dict) -> Path:
    ...
    # Write to temp file
    tmp = tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False, dir=str(ROOT / "backend" / "src" / "risk_engine"))
    yaml.dump(cfg, tmp, default_flow_style=False, sort_keys=False)
    tmp.close()
    return Path(tmp.name)

def main():
    base_rules = ROOT / "backend" / "src" / "risk_engine" / "rules.yaml"
    ...
    tmp1 = tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False, dir=str(ROOT / "backend" / "src" / "risk_engine"))
