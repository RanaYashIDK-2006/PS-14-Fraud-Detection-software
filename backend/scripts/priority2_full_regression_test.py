...
def test_rate_limiter_ip_keying():
    src = (ROOT / "backend" / "src" / "middleware" / "rate_limiter.py").read_text(encoding="utf-8")
...
def test_device_fingerprints_composite_pk():
    models_src = (ROOT / "backend" / "src" / "privacy_layer" / "models.py").read_text(encoding="utf-8")
...
def test_demo_seed_production_gate():
    verify_src = (ROOT / "backend" / "src" / "verification_service" / "main.py").read_text(encoding="utf-8")
...
def test_db_paths_absolute():
    ...
    has_resolve = "_PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent" in settings_src
    ...
    try:
        import os as _os
        _os.chdir(str(ROOT / "backend" / "scripts"))  # change to unrelated dir
        # Force reimport
...
def test_citations_illustrative():
    compare_src = (ROOT / "backend" / "scripts" / "compare_ml_systems.py").read_text(encoding="utf-8")
...
def test_sqlite_pool_scoping():
    shared_db = (ROOT / "backend" / "src" / "shared_db.py").read_text(encoding="utf-8")
