...
def fraud_report(response: Response) -> JSONResponse:
    ...
    report_path = Path(__file__).resolve().parent.parent.parent / "data" / "fraud_report_data.json"
...
def chameleon_results() -> JSONResponse:
    ...
    p = Path(__file__).resolve().parent.parent.parent / "data" / "updated_chameleon_results.json"
    if p.exists():
        try:
...
def monitor_metrics(response: Response) -> JSONResponse:
    ...
    if not recent:
        try:
            _rpt_path = Path(__file__).resolve().parent.parent.parent / "data" / "fraud_report_data.json"
            if _rpt_path.exists():
    ...
    try:
        manifest_path = Path(__file__).resolve().parent.parent.parent / "models" / "production" / "manifest.json"
    ...
    try:
        native_path = Path(__file__).resolve().parent.parent.parent / "models" / "production" / "altman_native" / "manifest.json"
